...
#python em_final_yaz.py --indir /data/soumen/ncc/glove300 --phrases /data/srijayd/local_data/f_r1_r2_th/th/fh1.csv --model /data/srijayd/models/emmodel/ --train True
...
class NccDiscrepancy(NonComp):
	...
	self.k=200    #prior belief on number of non-comps
	self.value,self.index=tf.nn.top_k(tf.transpose(self.errore), self.k, sorted=True)    #index of topk most erroreous entries
	idx,dummy=tf.nn.top_k(self.index, self.k, sorted=True)    #sort indexes in decending order
	...
	self.delta= tf.sparse_tensor_to_dense(tf.SparseTensor(tf.transpose(tf.to_int64(idx)),values=tf.ones(self.k),shape=[self.numDatInst]))    #generate a list in which only topk erroreous entry is 1 rest is zero
...
'''
	...
	print "Model path is ",tfModelDirPath
        ...
        tfModelPath = os.path.join(tfModelDirPath, 'compmodel194.tf')
