...
#python em_2cos.py --glove /data/srijayd/local_data/glove_embeddings/ --phrases /data/srijayd/local_data/f_r1_r2_th/th/fh1.csv --model /data/srijayd/models/emmodel/ --train True --alltest True
...
class NccDiscrepancy(NonComp):
	...
	with open(thCsvPath, mode='r') as th:
		...
		sPhrase = "_".join([row[0], row[1]]) #phrase in Glove is seperated by _ 
                iv1, iv2, iv3 = row[0] in self.termToTid, row[1] in self.termToTid, sPhrase in self.termToTid 
	...
	self.M1e = tf.placeholder(tf.float32, shape=(self.dim, self.dim))
	self.M2e = tf.placeholder(tf.float32, shape=(self.dim, self.dim))
	...
	matSize = self.dim * self.dim  
        stddev = 1. / math.sqrt(matSize)
        self.M1m = self.buildVariable([self.dim, self.dim], stddev, 'M1m') 
        self.M2m = self.buildVariable([self.dim, self.dim], stddev, 'M2m') 
	...
	for xiter in xrange(maxIters):
		for miter in xrange(2):		
			_,dlabel = sess1.run([self.trainopm,tf.transpose(self.labelm)],
						       ...
						       self.labelm: prev_labl	
                                                       })  #dlabel thing is just for debugging
			
		self.errs,cur_lab = sess2.run([self.errore,self.coslab],feed_dict={self.w1e: self.allw1,    
                                               self.w2e: self.allw2,
		...
		for i in range(33):
			sum1 = 0
			for j in range(30):
				sum1 += self.ally[idex_nc[30*i+j]]    #summation of y_score for 30 instances at a time
			print 30*(i+1)," ",sum1
		prev_labl=cur_lab
		'''	
