...
class NccDiscrepancy(NonComp):
	...
	diffvec1 = self.embeds[self.allw12[x]]-self.embeds[self.allw1[x]]
	...
	pred_classes_dev = self.clf.predict(self.X_dev)
	curr_accuracy = self.getAccuracy(confusion_matrix(self.cdY,list(pred_classes_dev)))

	print "Accuracy - ",curr_accuracy
        
	Cs = [.01,.02,.05,.1,.2,.5,1,2,5,10,20,50,100]
	gammas = np.linspace(0.0001,3,10)
	bestgamma = -1
	bestC = 0
	
	for i in range(0,len(Cs)):
		for j in range(0,len(gammas)):
			svm_accuracy = self.checkSVMAccuracy(Cs[i],gammas[j])
			print "Accuracy with C ",Cs[i]," and gamma ",gammas[j]," is ",svm_accuracy
			if(svm_accuracy > curr_accuracy):
				self.clf = svm.SVC(C=Cs[i],gamma=gammas[j])
				curr_accuracy = svm_accuracy
